...
#define		LINE_SIZE (16)		// Change for different line sizes

const int numberOfLines = CACHE_SIZE / LINE_SIZE;

int cache[numberOfLines][2];		//cache array;
...
cacheResType cacheSimDM(unsigned int addr)
{
	...
	if ((cache[index][0]==1)&(cache[index][1] == tag)) {		//if the line tag == address tag  << HIT >>
		return HIT;
	}
		else		//<< MISS >>
        {
...
cout<<"For line size: "<<LINE_SIZE<<endl;
    for (int g=1;g<=6;g++)
    {
	unsigned int hit =0;
	cacheResType r;
